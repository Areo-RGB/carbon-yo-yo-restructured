...
test('trimmed media starts protocol time at zero for both protocols', () => {
  ...
  assert.equal(protocolElapsedFromMediaMs('yoyoIR2', 0), 0);
  assert.equal(protocolElapsedFromMediaMs('yoyoIR2', 9_000), 9_000);
});
...
test('audio and protocol timeline conversion round-trips', () => {
  ...
  assert.equal(mediaElapsedFromProtocolMs('yoyoIR2', 9_000), 9_000);
});
